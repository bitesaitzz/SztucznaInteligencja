...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            left = y[:idx + 1]
            right = y[idx + 1:]
            left_freq = len(left) / len(y)
            right_freq = len(right) / len(y)
            left_pos_neg = np.bincount(left)
            right_pos_neg = np.bincount(right)
            gini_left = 1 - np.sum(np.square(left_pos_neg / len(left)))
            gini_right = 1 - np.sum(np.square(right_pos_neg / len(right)))
            gain = 1 - (left_freq * gini_left + right_freq * gini_right)
            if gain > best_gain:
                best_gain = gain
                best_idx = idx
