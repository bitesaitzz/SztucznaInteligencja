...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            return self.f_act(np.dot(x_data, self.W) + self.b)  # zastosowanie funkcji aktywacji na logits
...
def zad2_two_layer_net(student_id):
    ...
    #model.hidden_layer.W[:, 0] = [-1.5, 4.1]
    model.hidden_layer.W[:, 0] = [7,1] # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-7, 1]  # wagi neuronu h2
    model.hidden_layer.b[:] = [-1, 5]  # biasy neuronów h1 i h2
...
if __name__ == '__main__':
    ...
    student_id = 196643        # Twój numer indeksu, np. 102247
